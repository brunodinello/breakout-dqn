...
class DoubleDQNAgent(Agent):
    ...
    Uses the online network to select the next action and the target network to
    evaluate it, mitigating the maximization bias of vanilla DQN.
    """

    def __init__(self, gym_env, model_a, model_b, obs_processing_func, memory_buffer_size,
                 batch_size, learning_rate, gamma, epsilon_i, epsilon_f, epsilon_anneal_steps,
                 episode_block, device, sync_target=1000):
        ...
        self.online_net = model_a.to(self.device)
        self.target_net = model_b.to(self.device)
        self.policy_net = self.online_net  # alias used by Agent.save_parameters

        self.criterion = nn.MSELoss()
        self.optimizer = optim.Adam(self.online_net.parameters(), lr=self.learning_rate)

        self.sync_target = sync_target
        self.syncs_steps = sync_target
    ...
    def update_weights(self):
        ...
        q_s_a = self.online_net(states_tensor).gather(1, actions_tensor)

        with torch.no_grad():
            best_actions = self.online_net(next_states_tensor).argmax(dim=1, keepdim=True)
            q_next = self.target_net(next_states_tensor).gather(1, best_actions) * (1 - dones_tensor)
        ...
        self.syncs_steps -= 1
        if self.syncs_steps <= 0:
            self.syncs_steps = self.sync_target
